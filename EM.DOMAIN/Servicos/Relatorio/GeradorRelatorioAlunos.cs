...
namespace EM.DOMAIN.Servicos.Relatorio
{
    ...
    {
        public byte[] GerarRelatorio(List<Aluno> alunos, CidadeModel? cidade, string? uf, int? Sexo, string? Ordem, bool? linhasZebradas, bool horizontal)
        {
            ...
            using (MemoryStream ms = new MemoryStream())
            {
                ...
                if (cidade != null || Sexo.HasValue || uf != null)
                {
                    ...
                    if (uf != null)
                    {
                        ...
                        string filterCidade = $"• Estado: {uf}";
                        cb.ShowTextAligned(Element.ALIGN_LEFT, filterCidade, posX, posicaoY + alturaRetangulo - 10 - 10 - 20, 0);
                        posY -= 15;
                    }
                    if (cidade != null)
                    {
                        alunos = alunos.Where(a => a.Cidade!.Id_cidade == cidade.Id_cidade).ToList();
                        string filterCidade = $"• Cidade: {cidade.Nome}";
                        cb.ShowTextAligned(Element.ALIGN_LEFT, filterCidade, posX, posicaoY + alturaRetangulo - 10 - 10 - 20, 0);
                        posY -= 15;
                    }
                    if (Sexo.HasValue)
                ...
                //criar célula do cabeçalho com certos padrões
                ...
                foreach (Aluno aluno in alunos)
                {
                    ...
                    table.AddCell(aluno.Sexo == SexoEnum.Masculino ? "M" : "F".ToString());
